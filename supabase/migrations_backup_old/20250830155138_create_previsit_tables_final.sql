...
DROP TRIGGER IF EXISTS update_previsit_preparations_updated_at ON previsit_preparations;
CREATE TRIGGER update_previsit_preparations_updated_at 
    BEFORE UPDATE ON previsit_preparations 
    FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();

DROP TRIGGER IF EXISTS update_previsit_tool_selections_updated_at ON previsit_tool_selections;
...
BEGIN
    ...
    IF p_section_name IS NOT NULL AND p_section_data IS NOT NULL THEN
        ...
        END CASE;
    ELSE
        -- Update auto_save_data if no specific section
        UPDATE previsit_preparations 
        SET auto_save_data = p_section_data,
            last_activity = NOW()
        WHERE id = prep_id;
    END IF;
...
CREATE POLICY "Enable all operations for authenticated users" ON previsit_preparations
    FOR ALL USING (auth.role() = 'authenticated');

CREATE POLICY "Enable all operations for authenticated users" ON previsit_tool_selections
    FOR ALL USING (auth.role() = 'authenticated');
