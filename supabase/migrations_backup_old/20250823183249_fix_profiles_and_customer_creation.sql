...
-- Now create the fixed customer creation function
...
-- Also create validation function that checks against profiles
...
BEGIN
    ...
    -- Validate technician IDs against profiles
    IF form->>'primary_technician_id' IS NOT NULL AND form->>'primary_technician_id' != '' THEN
        IF NOT EXISTS (
            SELECT 1 FROM public.profiles 
            WHERE id = (form->>'primary_technician_id')::uuid
        ) THEN
            errors := errors || jsonb_build_object('field', 'primary_technician_id', 'message', 'Primary technician not found in system');
        END IF;
    END IF;
    ...
    IF form->>'secondary_technician_id' IS NOT NULL AND form->>'secondary_technician_id' != '' THEN
        IF NOT EXISTS (
            ...
            errors := errors || jsonb_build_object('field', 'secondary_technician_id', 'message', 'Secondary technician not found in system');
        END IF;
    END IF;
