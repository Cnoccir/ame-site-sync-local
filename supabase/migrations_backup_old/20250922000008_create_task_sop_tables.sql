...
-- Enable RLS but make it publicly readable (reference data)
ALTER TABLE task_sop_library ENABLE ROW LEVEL SECURITY;

-- Allow all users to read task/SOP library (reference data)
CREATE POLICY task_sop_library_read_policy ON task_sop_library
    FOR SELECT USING (true);

-- Only admins can modify (insert/update/delete)
CREATE POLICY task_sop_library_admin_policy ON task_sop_library
    FOR ALL USING (
        EXISTS (
...
CREATE TRIGGER update_task_sop_library_updated_at
    BEFORE UPDATE ON task_sop_library
    FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();

-- Insert sample CORE tasks and SOPs based on the CSV data
