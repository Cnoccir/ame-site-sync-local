...
CREATE POLICY "Users can view task sops" ON task_sops FOR SELECT USING (true);
CREATE POLICY "Authenticated users can manage task sops" ON task_sops FOR ALL USING (auth.uid() IS NOT NULL);
...
INSERT INTO task_sops (task_id, sop_title, sop_category, procedure_steps, visual_guides, additional_resources) 
...
CREATE OR REPLACE TRIGGER update_task_sops_updated_at 
BEFORE UPDATE ON task_sops 
FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();